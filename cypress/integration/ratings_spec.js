		...
		cy.fixture('auth_user_id.json').then((user) => {
			cy.readFile(Cypress.env('filePath').concat('/org_created.json')).then(
				(org) => {
					...
					//save rating
					cy.writeFile(Cypress.env('filePath').concat('/rating.json'), rating);
		...
		cy.readFile(Cypress.env('filePath').concat('/org_created.json')).then(
			(org) => {
				...
				cy.request({
					...
					cy.readFile(Cypress.env('filePath').concat('/rating.json')).then(
						(rating) => {
		...
		cy.fixture('auth_user_id.json').then((user) => {
			cy.readFile(Cypress.env('filePath').concat('/org_created.json')).then(
				(org) => {
					...
					//save rating
					cy.writeFile(
						Cypress.env('filePath').concat('/rating_service.json'),
						rating
					);
