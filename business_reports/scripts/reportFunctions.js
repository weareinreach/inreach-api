...
require('../../src/db');
const fs = require('fs');
const report_path = './reports/output/';
...
export async function getServiceNational(states, model, country, resultArray) {
...
export async function getServiceState(states, model, country, resultArray) {
	let service_tags = `services.tags.${country}`;
	for (const state of states) {
		let result = await model.aggregate([
			{
