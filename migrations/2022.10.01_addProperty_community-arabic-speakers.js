/**********************************************************************************
  ...
  * This file will add the community-arabic-speakers property
  * ********************************************************************************
	...
	try {
		const result = await mongoose.Organization.aggregate([
			...
			},
			{
				$project: {
					...
					properties: '$services.properties.lang-arabic'
				}
			}
		]);
		let bulkOperations = [];
		...
		result.forEach((org) => {
			updateOne = {
				...
				update: {
					$set: {
						'services.$[elem].properties.community-arabic-speakers': 'true'
					}
				},
		...
		console.log(
			'Migration to add the property community-arabic-speakers has executed'
		);
		process.exit(0);
	} catch (err) {
		console.log(err);
	...
	try {
		...
		result.forEach((org) => {
			updateOne = {
				...
				update: {
					$unset: {
						'services.$[elem].properties.community-arabic-speakers': ''
					}
				},
				arrayFilters: [{'elem._id': {$eq: org.service_id}}]
			};
		...
		console.log(
			'Rollback to remove the property community-arabic-speakers has executed'
		);
		process.exit(0);
	} catch (err) {
		console.log(err);
