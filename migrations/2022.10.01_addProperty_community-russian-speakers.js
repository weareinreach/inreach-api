/**********************************************************************************
  ...
  * This file will add the community-russian-speakers property
  * ********************************************************************************
...
async function runMigrationScript() {
	try {
		const result = await mongoose.Organization.aggregate([
			...
			},
			{
				$project: {
					...
					properties: '$services.properties.lang-russian'
				}
			}
		]);
		let bulkOperations = [];
		...
		console.log(
			'Migration to add the property community-russian-speakers has executed'
		);
		process.exit(0);
	} catch (err) {
		console.log(err);
...
async function runRollbackScript() {
	try {
		...
		console.log(
			'Rollback to remove the property community-russian-speakers has executed'
		);
		process.exit(0);
	} catch (err) {
		console.log(err);
